...
for (let i = 0; i < lines.length; i++) {
	...
	if (re01.test(lines[i]) &&
		...
		if (m2[1] == m4[1] && m3[1] == m5[1]) {
			...
			lines.splice(i, 6, lines[i+1].replace(/\*\//, '::1*/'),
							   lines[i].replace(/\*\//, '::1*/'));
			i++;
			...
			if ((new RegExp('\\W' + m2[1] + '\\W')).test(lines[i+5]) ||
				...
				lines.splice(i, 7, lines[i+1].replace(/\*\//, '::1*/'),
								   lines[i+5],
								   lines[i].replace(/\*\//, '::1*/'));
				i += 2;
			}
	...
	//	$k[$j++] = _AC; /*19477*/
		...
		if (m1[1] == 'Infinity' && m4[1] == m2[1]) {
			...
			lines.splice(i, 5, 
					lines[i+1].replace(/\*\//, '::2*/'),
					lines[i+3].substr(0, lines[i+3].length - m3[2].length) +
								m0[1] + m0[2].replace(/\*\//, '::2*/'));
		}
	// $j--;
	// $k[$j++] = ...
		...
		lines.splice(i, 2,
				m[1] + '$k[$j-1] = ' +
				lines[i+1].substr(m[0].length).replace(/\*\//, '::3*/'));
		i++;
	// $j -= 2;
	// $k[$j++] = ...
		...
		lines.splice(i, 2,
				m[1] + '$k[--$j - 1] = ' +
				lines[i+1].substr(m[0].length).replace(/\*\//, '::4*/'));
		i++;
		...
		for ( ; j < 20 && !refa.test(lines[i-j]); j++) {
			...
			if (mj) {
				...
				if (m1 && m1[1] == mi[2]) {
					...
					console.log(lines[i]);
					console.log('');
					let expr = lines[i-1].substr(m1[0].length).replace('*/', '::5*/');
...
fs.writeFileSync('src/bwipp.js', lines.join('\n'), 'utf-8');
