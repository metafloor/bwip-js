...
> customized the node module or integrated bwipjs into your code directly,
...
* Current bwipjs version is 1.0.0 (2016-06-15)
...
demonstrates all of the features of bwipjs.
...
A bwipjs barcode service is available online, ready to serve up barcode images
on demand.
...
```javascript
// Simple HTTP server that renders barcode images using bwipjs.
...
The bwipjs request handler only operates on the URL query parameters and
...
You can also use bwipjs to generate PNG images directly.
...
Only the first two options `bcid` and `text` are required.  The other bwipjs
...
compile bwipjs on your own, you will need to download the `develop` branch, 
